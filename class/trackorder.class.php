...
add_action( 'woocommerce_order_status_changed', 'bpax_track_order_details' );

function bpax_track_order_details( $order_id = null ) {
	$leapingDebug = false;
	$userIP       = getUserIpAddr();
	if ( $userIP == '69.201.31.171' ) {
		$leapingDebug = true;
	}
	...
	foreach ( $items as $item_id => $item_data ) {
		...
		if ( ! empty( $product_variation_id ) ) {
			...
			if ( get_post_meta( $item_data->get_product_id(), 'bpax_complex_or_simple', true ) == 'yes' && get_post_meta( $item_data->get_product_id(), 'bpax_complex_or_simple', true ) != '' ) {
				...
				$order_line_api_data['skuQuantities'][] = (object) array(
					'skuId'             => strval( $product->get_sku() ),
					'upc'               => null,
					'skuName'           => strval( bpax_retrieve_plain_title( $product->get_name() ) ),
				...
				foreach ( $add_on_meta_data as $meta_data ) {
					$sku_number = bpax_retrieve_sku_from_title( $meta_data->value );
					if ( ! empty( $sku_number ) ) {
						$order_line_api_data['skuQuantities'][] = (object) array(
							'skuId'             => strval( $sku_number ),
							'upc'               => null,
							'skuName'           => strval( bpax_retrieve_plain_title( $meta_data->value ) ),
